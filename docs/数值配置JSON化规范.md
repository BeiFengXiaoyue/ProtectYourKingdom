...
> ⛔→✅ **2026-09-11 政策两度变更（以第二次为准）**：早先裁定《整改方案》§7 P0-4「塔数值入 JSON」
> **取消**（`15f307f`，数值直接写死）；经再沟通后**同日推翻**——塔数值改走 **`config/tower.json`**
> （`util.balance.TowerBalance` 构造期读取），随 `feature/balancce-json` 合入 `dev`（见 §3.3 末行）。
> **本 3 键（炮塔溅射半径）保留现状、不回退**，仍走 `balance.json`，属 P1-1 已完成项。
...
> 对**本 3 键**不适用——**P0-4（塔数值 JSON 化）已于 2026-09-11 二次裁定启用**，但本 3 键由
> `balance.json` 承载（不经 `tower.json`），故仍为**长期现状**，不再计划"迁移回 `GameConfig`"。
...
| 塔 / 士兵数值（伤害/射程/冷却/士兵属性） | ✅ **`config/tower.json`**（`util.balance.TowerBalance` 构造期读取，缺省回退类内默认值） | ✅ **已纳入（2026-09-11 二次裁定，P0-4 启用）**：随 `feature/balancce-json` 合入 `dev`。**造价/升级链**与**溅射半径 3 键**不在 `tower.json`——前者留塔类常量，后者走本文件 `balance.json`（P1-1，**保留不回退**） |
...
| `docs/视觉尺寸配置规范.md` §五（`:146`） | “实体 HP/速度/造价等数值**仍归 GameConfig** 与实体构造器，一律不动” | ⚠️ **2026-09-11 起部分需修订**：P1-1 已把**溅射半径**纳入并改为**炮塔构造期直接读 `BalanceTable.runtime()`**；P0-4 二次裁定后**伤害/射程/冷却**改走 `config/tower.json`——实体构造器里的取值方式**已变**（不再纯由 Java 字面量决定），且两批均不经 `GameConfig`。**造价**仍留塔类常量，该部分继续成立 |
...
- **不新增第二套数值通道**：数值唯一源文件 = `config/balance.json`，唯一运行期入口 = `GameConfig`；不得再在别处写死这些数值；
  ...
  > 原因是塔实例不持有 `GameConfig` 引用、`Tower` 基类注入钩子属 A 侧未开放区域。**P0-4（塔数值 JSON 化）已于 2026-09-11 二次裁定启用**（塔数值走 `config/tower.json`，同样不经 `GameConfig`），本偏离为**长期现状**，不再计划"迁移回 `GameConfig`"。
- 不改实体类（`model/enemy/*`、`model/tower/*`、`model/ally/*`）内的索敌/移动/攻击逻辑——实体类**只接收注入数值**；
  ...
  > 该 3 个写死值同样**不再受本红线保护**。**其余塔/士兵数值中伤害/射程/冷却/士兵属性经 2026-09-11 二次裁定改走
> `config/tower.json`（`TowerBalance` 构造期读取），不再以 Java 常量表达；造价与升级链仍留塔类常量**。
- 不动塔升级链与精英塔（`Tower.upgrade()` 的 ×1.3/×1.2 规则已随该"数值式升级"实现一并删除、`Elite*Tower` 升级链已接线）——**其余**塔数值入 JSON **已启用**（2026-09-11 二次裁定：走 `config/tower.json`；溅射半径按 P1-1 保留，见上）；
