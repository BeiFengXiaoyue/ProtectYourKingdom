...
- 启动后为**开始界面 → 选关界面 → 战场**三屏流程（`view/StartScreen`、`view/LevelSelectScreen`，实现在 `Main` 里装配）；进入战场时游戏循环才启动，**选关屏顶栏可用「← 返回」回开始界面**（返回时停掉游戏循环，下次进战场重新启动）。战场内打完一关可用结束遮罩的「下一关」继续。
- 战场内可用 HUD 的「暂停」打开暂停菜单：**继续游戏 / 返回选关 / 重新开始**。**返回选关＝放弃本局**，再进所选关经 `switchLevelAt` → `clearBattlefield` 从头开始。
