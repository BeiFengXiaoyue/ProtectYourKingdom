...
- 启动后为**开始界面 → 选关界面 → 战场**三屏流程（`view/StartScreen`、`view/LevelSelectScreen`，实现在 `Main` 里装配）；进入战场时游戏循环才启动，**选关屏顶栏可用「← 返回」回开始界面**（返回时停掉游戏循环，下次进战场重新启动）。战场内打完一关可用结束遮罩的「下一关」继续，也可用结束遮罩的「返回选关」回选关屏。
- 战场内可用 HUD 的「暂停」打开暂停菜单：**继续游戏 / 返回选关 / 重新开始**；**终局（胜利 / 失败）遮罩同样提供「返回选关」**。**返回选关＝放弃本局**，再进所选关经 `switchLevelAt` → `clearBattlefield` 从头开始。
  ✅ **暂停时间基准已平移**：`IGameLoop.pause()` / `resume()` 会把两个"绝对时间"基准（波间倒计时 `countdownDeadlineNanos`、出怪锚点 `WaveManager.waveStartNanos`）按暂停时长整体后移——暂停跨过波间倒计时再继续**不会**提前开波、也拿不到"被吞掉"的倒计时；暂停中出怪再继续**不会**同帧涌出积压的出怪指令。其余计时（冷却 / 眩晕 / Boss 踩踏 / 兵营补位 / 移动）为逐帧递减，停循环即冻结。
