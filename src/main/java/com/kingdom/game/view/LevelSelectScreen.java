 ...
 *
 * 版式：顶栏（左标题 + 右侧「← 返回」 + 下分隔线）+ 磁贴网格区 + 提示行，整屏铺满（根 StackPane
 * 由装配层放进与战场屏同尺寸的场景，故菜单屏天然用满 700×636）。网格区 `VBox.setVgrow(ALWAYS)`
 * 吃掉剩余高度——**关卡再多也只在网格区内竖向滚动**，界面与窗口尺寸恒定。底图是羊皮纸战役地图
 * （`/assets/ui_select_bg.png` 铺满整屏，缺图回退沙金渐变）。
 *
 * 磁贴内容为「该关地图缩略图 + 关卡名」（缩略图按约定读 /maps/&lt;key&gt;/map.png）。
...
public class LevelSelectScreen {
    ...
    /** 「← 返回」文字色（羊皮纸调的深棕，与标题、关名同色） */
    private static final String COLOR_TEXT = "#5a3a1a";
    ...
    /** 打开选关界面时调用一次：按当前关卡列表重建磁贴（缩略图 + 关卡名） */
    public void refresh() {
        ...
        try {
            ...
            // maps/index.json 语法非法时 MiniJson 抛 IllegalArgumentException；MapLibrary.listMapsFromClasspath
            // 现已双捕获兜住（返回空表），这里保留为第二层兜底：将来列表接口若改回抛异常，也只提示、不崩窗口
